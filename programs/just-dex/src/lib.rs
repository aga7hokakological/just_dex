...
declare_id!("FaUPDdn2rBSXhQuKFioxtCeHGX4o7hvLdbNVX4Au26vn");
...
pub mod just_dex {
    ...
    pub fn add_liquidity(
        ...
        let cpi_program_token0 = ctx.accounts.token0_program.to_account_info();
        ...
        let cpi_program_token1 = ctx.accounts.token1_program.to_account_info();
        ...
        let cpi_program = ctx.accounts.lp_program.to_account_info();
    ...
    pub fn remove_liquidity(ctx: Context<LiquidityOperations>, amount: u64) -> Result<()> {
        ...
        let cpi_accounts_lp = Burn {
            mint: ctx.accounts.lp_program.to_account_info(),
            from: ctx.accounts.user.to_account_info(),
            authority: ctx.accounts.authority.to_account_info(),
        };
        let cpi_program_lp = ctx.accounts.lp_program.to_account_info();
...
pub struct InitializeDex<'info> {
    ...
    pub lp_program: Interface<'info, TokenInterface>,
